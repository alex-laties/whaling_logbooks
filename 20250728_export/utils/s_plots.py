...
def plot_global(df_coords, df_usable, Figures, start_year=1820, end_year=1890, save = True):
    """
    ...
    Args:
        ...
        Figures (str): Path to the directory where the figure will be saved.
    ...
    plt.title(f'Usable Entries with 1-5 Day (Tier 4) Infilling', fontsize=16, y=1.048)

    # Save figure
    stamp = datetime.today().date().isoformat()
    if save:
        os.makedirs(Figures, exist_ok=True)
        out = (os.path.join(Figures, f'Tier4_Usable_{start_year}_{end_year}_colormap_{stamp}.png'))
